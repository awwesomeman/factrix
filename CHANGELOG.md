...
- **p-value field and column names unified to `p_value` throughout** (#TBD). `MetricResult.p → MetricResult.p_value`; `InferenceResult.p → InferenceResult.p_value`. Output columns: `EvaluationResult.to_frame()` `"p" → "p_value"`; `compare()` `"{metric}_p" → "{metric}_p_value"`; `slice_joint_test()` `"p" → "p_value"`. (`SliceResult.to_frame()` already used `"p_value"` and is unchanged.) The `metadata["p_value"]` dict key used by all metric implementations is also unchanged. No transitional aliases — hard cut. (#472 introduced `MetricResult.p`; this corrects the direction.)
