...
- **Metric discovery and thin-group diagnostics** (#720). `fx.metrics_summary()` returns a Polars `(family, metric, summary)` catalog for browsing — the readable companion to `list_metrics()`'s full `MetricSpec`. Metric factories now expose their real `__signature__` (editors, `help()` and `inspect.signature` showed `(*args, **kwargs)`; dispatch internals stay hidden). `quantile_spread`'s thin-group advisory — the median cross-section split into `n_groups` buckets leaving fewer than `MIN_GROUP_ASSETS` assets per bucket — is now also a structured `WarningCode.THIN_QUANTILE_GROUPS` (not only a Python warning), and that warning suggests a concrete `n_groups` instead of a vague "reduce".

- **`fx.evaluate_horizons` — multi-horizon sweep that feeds the FDR layer** (#689). Convenience entry that runs `evaluate` across several overlap horizons of one raw panel and returns a flat `list[EvaluationResult]` — one entry per `(factor, forward_periods)` — ready for `compare` and `bhy(expand_over=("forward_periods",))`. Each horizon is rebuilt from the raw panel via `compute_forward_return` (not idempotent, so a horizon cannot be re-derived from an attached panel), collapsing the hand-rolled per-horizon loop that previously had to get the rebuild and result-flattening right by hand. `evaluate`'s single-horizon `dict` contract is unchanged; `forward_periods` is a non-empty `list[int]` of distinct positive horizons. Cross-horizon comparability is scale-only — the `÷N` in `compute_forward_return` aligns rank-IC, while signed-return-mean metrics carry a compounding bias that grows with `N`. Only the forward return is computed; per-horizon winsorize / abnormal-return stays on the explicit `evaluate` loop.
...
- **`spanning_alpha` p-value and single-series NW-HAC Wald used the wrong degrees of freedom** (#716). `spanning_alpha` derived the alpha p-value on `n − 1` df while its t-stat was built on the full design matrix (`n − 1 − n_base_factors`), biasing it anti-conservative as base factors accumulate; it now references the regression residual df. The single-restriction NW-HAC Wald tests in `ts_quantile_spread` / `ts_asymmetry` referenced the asymptotic χ² (over-rejects on short `T`) and now use the finite-sample `F_{r, T−k}` reference, matching the cluster-Wald paths.
- **`ic()` small-cross-section shortfall named by the wrong axis; `drop_reason` contradicted a zero drop** (#718). On a few-asset panel (every date below `MIN_IC_ASSETS`) `ic()` short-circuited with `insufficient_ic_periods` though the cause was too few *assets* per date; it now reports `insufficient_ic_assets` with `min_assets_required` and a redirect hint (genuine date shortfalls keep the periods reason). Separately, the drop-stat schema stamped the static criterion label even when nothing was dropped — `drop_reason` is now `null` at `drop_rate == 0`.
...
- **A `not_applicable` metric no longer aborts a `strict=True` battery** (#719). Under the default `strict=True`, a `not_applicable*` short-circuit (the type-routing verdict that a metric's signal *type* does not fit the factor — e.g. a continuous-magnitude metric on a discrete signal) previously raised and discarded the applicable results in the same call. It is now a soft outcome (NaN + `is_applicable=False`) while the applicable metrics still return; data shortages (`insufficient_*`), missing input/config (`no_*`), and structure mismatches stay strict failures. `strict=False` is unchanged.
...
- **Public-API examples / metric catalog realigned** (#717). Fixed a `compare()` example that passed the `evaluate()` dict instead of `list(results.values())`, regenerated the stale `to_dict()` metadata block, corrected the `compare()` `rank`-column description, made the README single-asset example self-contained, gave the `oos_decay` example enough points to return a real value, and added `k_spread` / `directional_hit_rate` to the metric-catalog docstring.
- **Design-rationale notes for the issue #715 review** (#721). `stat-keys-by-metric.md` updated `ts_quantile_spread` / `ts_asymmetry` from "Wald χ²" to the finite-sample Wald F now used; `architecture.md` records the `strict` / applicability split (type-routing `not_applicable*` is soft, deficiencies are loud); `statistical-methods.md` records two deliberately-retained estimator simplifications (within-date ICC between-variance, conservative; HAC mean `t_{n-1}` on the full overlapping sample, mildly anti-conservative).
