...
- **`factrix.multi_factor.bhy` returns `Survivors` instead of `list[FactorProfile]`** (breaking under v0.x). The new container exposes `.profiles` (input order), `.adj_q` (bucket-local BHY-adjusted p-values, aligned), `.q`, `.expand_over` (tuple of partition keys), and `.n_total` (per-bucket `m`, keyed by `expand_over_values`). Survivor membership is definitionally `adj_q <= q` — `bhy` calls `bhy_adjusted_p` once per bucket and derives the mask from those values, so survivor set and adjusted p-values cannot disagree on ties or boundary cases (the previous parallel `bhy_adjust` mask path is removed). `Survivors` ships `__repr__` / `_repr_html_` for Jupyter — three-column `identity | primary_p | adj_q` table, plus an `expand_over_values` column when buckets are declared. The container is procedure-agnostic; future Holm / Bonferroni / Romano-Wolf verbs will populate the same shape via their own `*_adjusted_p`. Migration: replace `survivors` with `survivors.profiles` for downstream list / iteration use. (#171)
