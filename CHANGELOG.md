...
- **Drop `Layer-A` / `Layer-B` stage labels from code and published docs** (#214). Stage labels (`Layer-A` / `Layer-B`, `first / second layer`, `curated wrapper` paired with `dispatcher` as a tier) are planning-tracker vocabulary whose meaning drifts as the roadmap shifts; docstrings and `docs/` pages are bound to a release and silently rot when those labels rename. StatCode comments in `factrix._codes`, slice-test contract markers in `factrix.metrics.{hit_rate, monotonicity, caar}`, `_stats.{multiple_testing, slice_policy}`, and the `factrix.stats` Estimator docstrings (`WaldNWCluster` / `WaldTwoWayCluster` / `BlockBootstrap`) now describe behaviour functionally — `slice-test verb`, `slice-test Estimator`, `paired-diff slice test`. Same rewrite on `docs/api/stats.md`, `docs/api/index.md`, `docs/api/by-slice.md`, `docs/guides/regime-analysis.md`; `docs/api/metrics/ic.md` additionally drops the dead `regime_ic` member entry left over from #217. Test docstrings + `tests/stats/test_layer_b_estimators.py` (renamed to `test_slice_test_estimators.py`) follow the same rule. The contributing guide gains a §8 Terminology subsection codifying the convention so the rewrite does not regress. Pure docstring / docs / test-name refactor — no public API change, no behavioural change.
