...
- **Sample-size validation converged into one declarative subsystem** (#585, #587, #588, #594, #597). `SampleThreshold` now spans four shape axes — `periods` / `assets` / `pairs` / `events` (each `min_*` / `warn_*`) — and `DataProperties` exposes the matching `n_events` (non-zero factor observations) alongside `n_periods` / `n_assets` / `n_pairs`, so `inspect_data` pre-flights every declared floor on the same `n_<axis> < min_<axis>` contract. Metrics declare a floor once and enforce it through the shared `_enforce_min_floor(..., axis=...)` helper instead of ~15 hand-copied short-circuits; the event-driven metrics (`caar`, `bmp_test`, `corrado_rank`, `mfe_mae_summary`, `event_hit_rate` / `event_ic` / `profit_factor` / `event_skewness`) gate on the new events axis. Floors that depend on a metric's own parameters are declared via the `sample_threshold_for` hook (`ic`, `caar`, `turnover`) — returning a real `SampleThreshold` resolved at the default config — rather than an empty placeholder, so `inspect_data` can pre-flight them too. Threshold constants follow the `MIN_<DIMENSION>_<TIER>` grammar with the dimension token (`PERIODS` / `ASSETS` / `PAIRS` / `EVENTS`) mandatory.
...
- **`pooled_beta(driscoll_kraay=True)` + `DriscollKraay` estimator** (#537). Optional [Driscoll & Kraay (1998)](https://github.com/awwesomeman/factrix/issues/537) cross-section-robust HAC standard errors on the FM pooled-OLS path, for small / cross-sectionally correlated panels where a one-way date cluster understates SE and inflates significance. DK sums the per-observation OLS scores `x_it·e_it` cross-sectionally within each period and runs a Bartlett-kernel HAC (NW1994 auto-bandwidth on the period count) over the resulting time series before sandwiching with `(X'X)⁻¹` — robust to arbitrary contemporaneous cross-sectional correlation (and serial correlation up to the bandwidth). `pooled_beta` gains keyword-only `driscoll_kraay: bool = False` (clustered SE stays the default; existing callers unaffected) and `driscoll_kraay_lags: int | None = None`; the chosen path is recorded in `metadata["se_method"]` (`"driscoll_kraay"`) with `n_periods` / `driscoll_kraay_lags`, uses `df = T_periods − 1`, emits `WarningCode.UNRELIABLE_SE_SHORT_PERIODS` below 30 periods, short-circuits below 3 periods (HAC undefined), and raises `ValueError` if combined with `two_way_cluster_col`. Surfaced through the estimators surface: `factrix.stats.DriscollKraay` (selection-only base `Estimator`, applicable to `(INDIVIDUAL, DENSE)`, listed by `list_estimators`, `emits_for → StatCode.P_DK`) and the lowercase `factrix.estimators.driscoll_kraay(X, resid, time_ids, *, lags)` callable; numerics in `factrix._stats.hac._driscoll_kraay_cov` / `_bartlett_lrcov`. `StatCode.P_DK` is added as a single p-code (no `T_DK`) — `DriscollKraay` is a reserved selection-only estimator whose t-stat lives in `pooled_beta`'s `MetricResult`, not `profile.stats` — and is the deliberate minimal extension of the over-budget StatCode enum (the next inference algorithm must trigger the structured-shape redesign first).
...
- **`EvaluationResult` / `MetricResult` / `Warning` result dataclasses** (#441). The unified result-type group that #438 / #442 will surface from the DAG executor lands as types-only in this release; runner wiring (replacing `FactorProfile` / `MetricsBundle` returns) is the follow-up. `EvaluationResult` holds bundle-level identity (`factor` / `axes` / `mode` / `n_obs` / `n_assets`) plus a `MetricResult` and a flat `list[Warning]`. `MetricResult` mirrors the `MetricGroups` shape (`applicable` / `primary` / `diagnostic` as `list[MetricSpec]` partitions) and exposes dict-like access (`__getitem__` / `keys` / `values` / `items` / `__iter__`) to the produced `MetricResult` instances, all keyed by metric name. `Warning` is a flat `(code, source, message)` dataclass — `source=metric_name` for per-metric warnings, `source=None` for bundle-level. Serialisers: `EvaluationResult.to_frame()` for long-form `pl.DataFrame` (stable schema for parquet stacking across factors), `to_dict()` for JSON-friendly nested dicts (NaN / Inf scrubbed to `None`), and `_repr_html_()` for Jupyter. The DoD `plan: str` field is deferred to #442 where the DAG executor that populates it actually exists (no placeholder string).
- **`MetricResult.spec: MetricSpec | None`** (#441). Back-pointer to the declaring `MetricSpec`. Default `None` so all existing call sites stay valid; runners stamp the field at dispatch time so downstream consumers can recover the spec without a name-keyed lookup.
...
- **Event-axis metadata keys renamed for token consistency** (#607). `caar()` metadata `n_event_dates` → `n_event_periods` (aligning with the repo's `periods` time-axis token) and `n_sampled` → `n_obs_sampled` (aligning with the `fx.inference` non-overlapping convention, which `ic()` already reads); `caar()`'s short-circuit `reason` likewise becomes `insufficient_event_periods`. `clustering_hhi` metadata `n_event_dates` → `n_event_periods` and `effective_n_dates` → `effective_n_periods` for the same token consistency. Metadata-key rename only — the statistics are unchanged.
...
- **`list_estimators()` retired its `scope` / `density` filter — now a no-arg overview API** (#255). Breaking signature change: `list_estimators(scope, density, *, format, with_import)` → `list_estimators(*, format, with_import)`. Following the architectural decision in #498 (where `list_metrics` became a pure no-arg overview), `list_estimators` returns every registered `Estimator` regardless of cell context; per-cell applicability stays each `Estimator.applicable_to(scope, density)`'s reserved contract and is no longer filtered here. The `IncompatibleAxisError` empty-cell raise path is removed (an empty registry now returns `[]`). Calling `list_estimators(scope, density)` raises `TypeError`. The `format` / `with_import` knobs and JSON-row schema are unchanged. Docs (`list-estimators`, `stats`, `errors`, `estimator-alternatives`, API index) and `llms-full.txt` updated in lock-step.

- **`MetricSpec.sample_threshold` populated across all 35 public metrics and short-circuit logic updated** (#456). Every public metric spec now declares its sample thresholds (e.g. `min_periods`, `min_assets`, `min_pairs`) declaratively in the `@metric` decorator instead of hardcoding or importing global constants. Metric implementations retrieve these thresholds dynamically from their own class/function attributes (`cls.sample_threshold` / `self.sample_threshold`), facilitating central management and pre-flight validation.
...
- **Single-metric result type unified: `MetricOutput → MetricResult`; container `MetricResult → MetricResultGroup`** (#472). The single-metric return type is renamed `MetricOutput → MetricResult` (aligning with the `EvaluationResult` naming family) and moves from `factrix._types` into `factrix._results` so every result dataclass lives in one module; it is now `frozen=True, slots=True` and gains a typed `p_value: float | None` field promoted from `metadata["p_value"]` (the raw key stays populated for tool context). The former container `MetricResult` (the per-cell `applicable` / `primary` / `diagnostic` group plus dict-like access) is renamed `MetricResultGroup`. `MetricResult.p_value` is the single read path for the p-value: `EvaluationResult.to_frame()` / `to_dict()` / `_repr_html_()`, `factrix.compare`, and the BHY family resolver all read it. `SliceResult.to_frame()` drops its `name` column (slice outputs carry no spec); on the executor path the metric name is carried on `MetricResult.name` (stamped at dispatch). No transitional aliases — hard cut.

- **Result types decoupled from `MetricSpec`** (#472). `MetricResult` no longer carries a `spec: MetricSpec | None` back-pointer; instead it holds `name: str` (the metric name, stamped by the DAG executor at dispatch, empty string `""` for free-standing primitive calls). `MetricResultGroup.applicable` / `primary` / `diagnostic` change from `list[MetricSpec]` to `list[str]` (metric names). This removes the result module's compile-time dependency on the metric registry so result dataclasses stay importable without pulling in spec metadata. Consumers reading `result.spec` must switch to `result.name`; consumers iterating the partition lists now receive name strings rather than spec objects (`to_dict()` / `_repr_html_()` already keyed by name). The internal `_dag._stamp_spec()` helper is gone. Six enums added to `factrix._axis` consolidating all spec metadata into typed fields: `Aggregation` (CS_THEN_TS / TS_THEN_CS / TS_ONLY / EVENT_TIME / CS_SNAPSHOT / RETURN_SPANNING), `TestMethod` (T / BINOMIAL / RANK / CHI2 / DESCRIPTIVE), `SEMethod` (HAC / CLUSTER / HETEROSKEDASTIC / OLS / BUILT_IN / NONE), `SpecRole` (METRIC / PIPELINE), `InputShape` (PANEL / SERIES / SCALAR), `OutputShape` (SCALAR / SERIES / PANEL). `MetricSpec` fields changed: `agg_order: str → aggregation: Aggregation`, `inference: str → test_method: TestMethod + se_method: SEMethod` (two orthogonal fields), `input_kind: Literal["panel","scalar"] → input_shape: InputShape`, `visibility: Visibility → role: SpecRole`, `sample_floor → sample_threshold` (required, defaults to `SampleThreshold()` all-None instead of `None`). Added `output_shape: OutputShape` with invariant `role=METRIC → output_shape=SCALAR` enforced in `__post_init__`. Removed `primitives: tuple[str, ...]`. `list_metrics(format="json")` keys updated: `agg_order → aggregation`, `inference_se → test_method + se_method`, `input_kind → input_shape`. `public_specs()` and `DagExecutor` dispatch filter on `role is SpecRole.METRIC` instead of `visibility is Visibility.PUBLIC`. All 19 public metric modules and conftest/test files updated in lock-step. `Visibility` enum retained for backward compatibility.
- **Axis naming convergence: `FactorSignal → FactorDensity`, `PanelMode → DataStructure`, `CONTINUOUS → DENSE`** (#478). Two axis enum names carried UX friction: `FactorSignal` reads as a tautology to quant practitioners (factor and signal are near-synonyms in the literature), and `PanelMode.PANEL` was self-referential while `PanelMode.TIMESERIES` contradicted the prefix. Renamed in full: `FactorSignal → FactorDensity`, `FactorSignal.CONTINUOUS → FactorDensity.DENSE` (string value `"continuous" → "dense"`), `FactorSignal.SPARSE` kept, `PanelMode → DataStructure` (values `PANEL` / `TIMESERIES` kept). Connected surface: `PanelProperties.signal / mode → density / structure`; `PanelReasoning.signal_reason / mode_reason → density_reason / structure_reason`; `Cell.density / structure` field names; `Cell.matches(scope, density, structure=None)` signature; `cell()` factory positional `density` + kwarg `structure=`; `EvaluationResult.cell` tuple second/third type annotations; `to_dict()` JSON keys `"signal" → "density"` and `"mode" → "structure"`; `_detect_signal → _detect_density`, `_detect_mode → _detect_structure`; `__all__` swaps `FactorSignal → FactorDensity`; `DataStructure` stays out of `__all__` (derived axis). No transitional aliases — pure rename, hard cut.
...
- **`EvaluationResult.cell` third axis: `Metric | None` → `PanelMode`; top-level `mode` field removed** (#473). The result tuple now carries `(scope, signal, mode)` so the three axes are the *observed* cell — same shape as `MetricSpec.cell` (where `mode` is the pattern axis), with mode rendered as a concrete `PanelMode` value rather than a wildcard. `to_dict()` JSON moves the mode value into `cell.mode` (the prior top-level `"mode"` key is gone); `_repr_html_` collapses the separate mode row into the cell-tuple row. The single-source-of-truth for "what mode did this evaluation run under" is now `result.cell[2]`.
...
- **`Estimator.emits_for(scope, signal, metric)` → `emits_for(scope, signal)`** (#473). The `metric` argument was unused — every estimator implementation (`NeweyWest`, `HansenHodrick`, `GMM`, `BlockBootstrap`, `WaldNWCluster`, `WaldTwoWayCluster`) prefixed it `_metric` and returned a fixed `StatCode`. The cell-key role the parameter once played was retired by the per-`MetricSpec` DAG dispatch in #448.
...
- **`EvaluationResult.axes` field renamed to `cell`** (#468). Pure rename — no behaviour change. The `(scope, signal, metric)` tuple stamped on each result is conceptually a *cell* (an axis-value combination identifying a dispatch unit) — the same noun `MetricSpec.cell: Cell` already uses. The prior `axes` name conflated the *axis* (single enum dimension) and *cell* (tuple of axis values) vocabularies; even the original docstring read `"axes: (scope, signal, metric) tuple of the dispatched cell"`, conceding the field held a cell. `to_dict()` JSON key and `_repr_html_` row label both move from `"axes"` to `"cell"` in lock-step. `ConfigError` / `InsufficientSampleError` docstring prose ("axis-tuple miswire" / "nearest-legal cell axes") unified to `cell`-noun usage. No transitional alias.
...
- **`fx.evaluate` signature** (#448): `(panel, cfg, factor_cols=...)` → `(panel, *, metrics: list[MetricSpec], factor_cols: list[str], forward_periods: int | None = None) -> list[EvaluationResult]`. Caller supplies the metric set explicitly via `metrics=` (a list of `MetricSpec` objects resolved through `fx.spec_by_name()` or registered via `fx.metric_spec` + `factrix.metrics.register`); the DAG executor closes the `requires` graph, runs batchable producers once across the factor batch, and returns one `EvaluationResult` per `factor_cols` entry in input order. The new entry adds a panel-mode applicability gate: passing a primary metric whose `cell.mode` disagrees with the panel's derived mode (the canonical case is information coefficient (IC) on a single-asset panel) raises a `UserInputError` listing `inspect_panel` as the pre-flight path instead of returning a bundle of NaN short-circuits.
...
- **`MetricResult.n_obs` first-class field** (breaking, #248). Promoted from a metadata dict key to a first-class dataclass field — `n_obs: int | None = None` — so user / AI-agent consumers reach the metric primitive's sample size with `output.n_obs` instead of `output.metadata.get("n_obs")`. Builds on the `n_observed → n_obs` metadata rename from #246. `_short_circuit_output(name, reason, n_obs=n, ...)` callers (~38 sites) auto-route via kwarg-only signature change; direct `MetricResult(...)` constructions in `factrix/metrics/spanning.py` and `factrix/metrics/fama_macbeth.py` are updated to pass `n_obs=` at the top level instead of nesting under `metadata={"n_obs": ...}`. `__repr__` now surfaces `n_obs=` between `value=` and `stat=` when populated. Same family name as `FactorProfile.n_obs` but scoped per metric primitive (single-stage estimator count) rather than per dispatched cell (final-stage test denominator).
...
- **`FactorProfile.diagnose()` schema overhaul** (breaking, #246). Six UX gaps in the structured triage interface land in one schema change. (1) **Four sample axes** replace the previous polymorphic `n_obs` + `n_assets` pair: `n_obs` (cell-canonical final-stage test denominator — semantics unchanged), `n_pairs` (non-null `(period, asset)` pair count, first-stage), `n_periods` (unique periods in raw panel), `n_assets` (unique assets, semantics unchanged). Each axis answers one question and never overlaps with another; a small `n_obs` is now disambiguated by the three companion axes without reverse-engineering the cell. (2) **`primary_stat` / `primary_stat_name` family** added top-level — `primary_stat: float | None` carries the test statistic value paired with `primary_p` (e.g. `t_nw` value for an NW HAC t-test), `primary_stat_name: str` slugs the `stats` key (e.g. `"t_nw"`) so the user can connect `primary_p` to its `stats` entry without consulting the procedure registry. Generic across statistic families: the `None` arm handles future empirical-p primaries (block bootstrap) where there is no test stat. Invariant `stats[primary_stat_name] == primary_stat` (when not `None`) is pinned in docstring. (3) **`cell` group** wraps the dispatch coordinate — `{"scope", "signal", "metric", "mode"}` — so consumers identify which procedure ran without grepping `config`. (4) **Reader-flow key order**: `identity` → `context` → `cell` → sample axes → `primary_p` / `primary_stat` / `primary_stat_name` → `warnings` / `info_notes` → `stats` / `metadata`. (5) **`n_observed` → `n_obs` metadata key rename** across ~38 `factrix/metrics/*.py` short-circuit sites — the two first-class surfaces (`FactorProfile` and `MetricOutput`-metadata) now share the same family name; the `MetricOutput.n_obs` first-class field promotion is tracked separately at #248. (6) **`SuggestConfigResult.diagnose()` "symmetric" docstring** is downgraded to scope-limited — the two surfaces share `warnings` serialisation but answer structurally different questions; no schema change there. Docs (`api/factor-profile.md`, `development/architecture.md`, `llms-full.txt`) are rewritten in lockstep; bare `N` / `T` letter codes in `factor-profile.md` are replaced with full axis names to reduce reader cognitive load.
  ...
  # - MetricResult.metadata key "n_observed" → "n_obs" across metric primitives.
  ```
...
- **GMM path and estimator discovery removed** (#569). The unused public functions `list_estimators`, `get_estimator`, `UnknownEstimatorError`, and the entire GMM path (`GMM`, `MomentEstimator`, `GMMResult`) are removed.
  - *Migration Note*: For custom estimator resolution, use per-metric introspection (`applicable_inference` list) rather than the global registry.
- **`StatCode` and `InfoCode` enums retired** (#573). The dead enums and `Estimator.emits_for` dispatch are removed as `FactorProfile.stats` has been cleaned up.
  - *Migration Note*: Reference raw strings in result keys or use the structured `InferenceResult` properties directly.
- **`test_method` and `se_method` fields dropped** (#574). The phantom `test_method` and `se_method` fields are removed from `MetricSpec` and the `@metric` decorator as they had no consumers.
  - *Migration Note*: Remove `test_method` and `se_method` arguments from all custom `@metric` definitions.
...
- **Public retirement sweep — `AnalysisConfig`, `FactorProfile`, `MetricsBundle`, `SuggestConfigResult`, `Metric` enum (out of `__all__`), `run_metrics` / `run_metrics_iter` / `run_metrics_chunked`, `evaluate_iter` / `evaluate_chunked`, `suggest_config` / `describe_analysis_modes`, `MissingConfigError` / `ModeAxisError` / `RunMetricsError`, `InfoCode.SCOPE_AXIS_COLLAPSED`** (#448). Replaced by the DAG-executor-backed `evaluate` (new signature in `### Changed`), `EvaluationResult` / `MetricResult` / `Warning` (#441) for results, `inspect_data` / `DataInspection` (#443, #527) for pre-flight introspection, and the `factrix.estimators` namespace (#444) for inference-method selection. The internal `_run_metrics`, `_chunk_size`, `_describe`, `_evaluate`, `_procedures`, `_registry`, `_analysis_config`, `_profile` modules and `_metric_index._AUTO_DISCOVER_EXCLUDED` dict go with the public surface. `InfoCode.SCOPE_AXIS_COLLAPSED` tracked a legacy dispatcher routing collapse that no longer happens under the DAG executor — the `InfoCode` enum stays exported as the home for future neutral notes but has no current members.
