...
- **`MetricResult.name` and `MetricResult.significance` fields** (#472, was `MetricOutput`). `name` is recovered from `spec.name` (`None` for direct primitive calls — the caller already holds the metric it invoked); `significance` is derivable from `p` via `factrix._stats._significance_marker`. `EvaluationResult.to_frame()` drops its `significance` column and `SliceResult.to_frame()` drops its `name` column accordingly.
