...
- **`MetricSpec.emitted_name` field + `factrix._metric_index.emitted_name_of()` helper** (#460). Bridge field that mapped `MetricSpec.name` (function name) to a divergent `MetricOutput.name` (output label) — no longer needed once the 5 callables are renamed so both identities coincide. `list_metrics(format="json")` output drops the `"emitted_name"` key from each row.
