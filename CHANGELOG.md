...
- **`MetricSpec.sample_threshold: SampleThreshold`** (#443, #456, #472). New field carrying per-metric pre-flight thresholds (`min_periods` / `warn_periods` / `min_assets` / `warn_assets` / `min_pairs` / `warn_pairs`). `inspect_data` evaluates these against `DataProperties` to surface per-metric usability without running the metric.
