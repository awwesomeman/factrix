...
- **`greedy_forward_selection` prunes dead candidate refs per iteration** (#436). Internal hygiene pass on `factrix.metrics.spanning.greedy_forward_selection`: the intermediate `all_arrays` union dict is released right after `base_arrays` / `candidate_arrays` are built; selected factors are `pop`'d out of `candidate_arrays` instead of left to shadow the now-owning `selected_arrays` entry; backward-eliminated factors release their last ref as a free side effect. Pins the invariant `candidate_arrays.keys() == remaining` at every step (new `test_candidate_dict_pruned_when_factor_selected` spy verifies). At S4 scale (50 candidates × 1250 dates) the RSS saving is dwarfed by panel construction and is not measurable; at 1000-factor candidate pools the released numpy buffers (~16 MB) and dict-entry overhead become observable, and the invariant alone is worth pinning regardless. No public-API or behavioural change; `selected_factors` / `eliminated_factors` / `all_candidates` content unchanged.
