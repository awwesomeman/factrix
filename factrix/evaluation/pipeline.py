...
``factrix._api.evaluate`` / ``evaluate_batch``; this module is the
...
def _validate_n_assets(df: pl.DataFrame, factor_type: str) -> None:
    """Validate panel shape; delegates to ``factrix._validators``."""
...
def _check_date_dtype_match(
    ...
    *,
    ...
    factrix is TZ-agnostic — any Datetime time_unit / time_zone is
...
def _augment_level2_intermediates(
    ...
    from factrix.evaluation.profiles._base import _stash
    from factrix.metrics.ic import multi_horizon_ic, regime_ic
...
def _build_macro_common_artifacts(
    ...
    from factrix.metrics.ts_beta import compute_ts_betas, compute_rolling_mean_beta
