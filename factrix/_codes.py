...
_WARNING_DESCRIPTIONS.update(
    {
        ...
        WarningCode.OVERLAPPING_PREDICTIVE_INFERENCE: "predictive_beta was run with overlap_periods > 1, where the reported slope HAC test is not calibrated at a nominal 5%. metadata['stambaugh_adjusted'] identifies the path: the Amihud-Hurvich-adjusted HAR test measures 7.5-14.5% null rejection, while False identifies a plain OLS-Newey-West fallback with no fallback-specific overlapping null-size band. The estimate and p-value are returned, but use a raised significance hurdle and compare an h=1 or genuinely non-overlapping design. This is distinct from EVENT_WINDOW_OVERLAP, which removes overlapping event-study observations before testing.",
