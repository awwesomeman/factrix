...
| `positive_rate` exact binomial | 2.3–4.7% in most iid cells; one IC-pipeline cell measured 8.0% at `n=240`. On AR null series at `T=200`, size was 5.2% (`phi=0`), 11.5% (`phi=0.3`), 14.8% (`phi=0.4`), 20.2% (`phi=0.5`), 25.2% (`phi=0.6`) and 54.8% (`phi=0.9`) over 400 draws. With the hit-series threshold, the screen fired on 0.5% / 44.8% / 78.5% / 96.8% / 99.8% / 100% of draws at `phi=0` / 0.3 / 0.4 / 0.5 / 0.6 / 0.9. | Exact-binomial discreteness is conservative under independent trials but does not protect against serially dependent hit indicators. `serial_correlation_detected` identifies persistence left after the overlap stride; the p-value is returned under the project-wide advisory contract but is not calibrated in that regime. **The screen reads the dichotomised hit sequence, whose lag-1 autocorrelation is `(2/pi) arcsin(phi)` for a zero-mean Gaussian AR(`phi`) input — measured 0.194 at `phi=0.3` and 0.263 at `phi=0.4` against a predicted 0.194 / 0.262.** `POSITIVE_RATE_HIT_AUTOCORR = (2/pi) arcsin(PERSISTENT_SERIES_AUTOCORR) ≈ 0.194` restores the population warning boundary to `phi=0.3` on that calibration grid. It is a metric-specific warning threshold, not a claim that arbitrary non-Gaussian factor series have a recoverable latent Gaussian correlation. The exact-binomial estimator and p-value are unchanged. |
...
The `positive_rate` threshold uses the Gaussian sign-correlation identity from
[Hurt (1973)][hurt-1973]. Its measured trigger rates, rather than that identity
alone, define the supported calibration boundary.
