...
| `positive_rate` exact binomial | 2.3–4.7% in most iid cells; one IC-pipeline cell measured 8.0% at `n=240`. On AR null series at `T=200`, size was 5.2% (`phi=0`), 25.2% (`phi=0.6`), and 54.8% (`phi=0.9`) over 400 draws. | Exact-binomial discreteness is conservative under independent trials but does not protect against serially dependent hit indicators. `serial_correlation_detected` identifies persistence left after the overlap stride; the p-value is returned under the project-wide advisory contract but is not calibrated in that regime. |
