...
| `positive_rate` exact binomial | 2.3–4.7% in most iid cells; one IC-pipeline cell measured 8.0% at `n=240`. On AR null series at `T=200`, size was 5.2% (`phi=0`), 11.5% (`phi=0.3`), 14.8% (`phi=0.4`), 20.2% (`phi=0.5`), 25.2% (`phi=0.6`) and 54.8% (`phi=0.9`) over 400 draws; the screen fired on 7.0% / 28.8% / 61.0% / 91.2% / 100% of those draws respectively. | Exact-binomial discreteness is conservative under independent trials but does not protect against serially dependent hit indicators. `serial_correlation_detected` identifies persistence left after the overlap stride; the p-value is returned under the project-wide advisory contract but is not calibrated in that regime. **The screen reads the dichotomised hit sequence, whose lag-1 autocorrelation is `(2/pi) arcsin(phi)` for a Gaussian AR(`phi`) input — measured 0.194 at `phi=0.3` and 0.263 at `phi=0.4` against a predicted 0.194 / 0.262.** Against the shared `PERSISTENT_SERIES_AUTOCORR` floor of 0.3 that puts the effective trigger at `phi > sin(0.3*pi/2) = 0.454`, so `phi` in 0.3-0.45 is oversized (roughly 2-3x nominal) and mostly unflagged. Screening the underlying continuous series instead, or lowering the floor to the dichotomised equivalent, would close that band — both change when the code fires and need their own calibration. |
