...
- **在本框架的應用：** v3 放棄 composite score 的核心理由之一。改用 gate-based pass/fail 設計，gates 是篩選工具而非最佳化目標。
...
- **在本框架的應用：** v3 scope 收窄至截面個股因子的核心依據。事件訊號應使用 event study 方法論獨立評估，不應強行套入截面因子的 gate 框架。兩者共享基礎設施（MLflow、Dashboard UI）但不共享評估邏輯。
...
- **在本框架的應用：** `compare(bhy=True)` 的理論基礎。BHY（下條）是 BH 的推廣，但核心演算法相同。factorlib 的實作使用 p-values 作為輸入（而非 t-stats），確保不同 factor type 的不同檢定類型可以統一處理。
...
- **在本框架的應用：** `compare(bhy=True)` 使用 BHY 精確校正（而非固定 t > 3.0），因為篩選 session 中 N 已知。Harvey 的 t > 3.0 保留作為文獻參考，不作為 factorlib 的門檻。
...
- **在本框架的應用：** 支持閘門機制（OOS Decay VETO、t-stat skip）的設計——不依賴單一指標，而是多層過濾。
...
- **在本框架的應用：** Gate 1 P1 BHY 連續校正的理論依據——從固定門檻升級為連續校正時，本文論證了為什麼離散階梯門檻不夠原則性。詳見 [§13](#13-統計穩健性與交易成本)。
...
- **在本框架的應用：** 支持 multi-split Gate 2 改進——使用 3+ 個切分點（而非單一 80/20）計算 OOS Decay ratio 取中位數，降低 regime-change 敏感度。
...
- **在本框架的應用：** 支持 `redundancy_matrix()` 的設計——因子個別有效但彼此冗餘是比 false discovery 更常見的問題。同時支持 BHY 作為 compare 欄位（資訊性）而非 Gate（攔截性）的設計決策：多重檢定問題是真的，但不應過度保守地攔截所有 marginally significant 因子。
...
- **在本框架的應用：** 如果未來實作 `factorlib/tools/panel/fama_macbeth.py` 或 macro 因子評估路徑，此文件提供直接可參考的實作模式。
...
- **在本框架的應用：** 支持 v3 Gate 1-3 逐市場獨立運行的設計——跨市場有效性應作為附加品質信號，而非取代本地評估。
...
- **在本框架的應用：** 因子是普遍的（pervasive：在多個區域出現）但定價機制是本地的（local：各區域的因子溢價大小和結構不同）。支持「本地 Gate 為主、跨市場一致性為輔」的分層設計。
...
- **在本框架的應用：** 日本的例外說明「Pervasive ≠ Identical Everywhere」——同一因子在不同市場可能有不同的表現模式。跨市場 Gate 通過數應容許部分市場未通過。
...
- **在本框架的應用：** 對 Gate 2 (OOS_Decay) 的跨市場含義：台股或其他新興市場的 OOS decay 可能天然低於美股，因為套利效率較低。0.5 門檻在不同市場的嚴格程度不同——但這是市場結構差異，不是門檻設計問題。
...
- **在本框架的應用：** 支持以超長歷史 + 跨資產類別證據作為因子「真實性」的判據。Dual replication 設計可與 v3 Gate 2 的 IS/OOS 概念對應。
...
- **在本框架的應用：** 理論上支持「本地 Gate 為主 + 跨市場信號為輔」的分層架構——兩者提供互補資訊，不應只取其一。
...
- **在本框架的應用：** 如果未來需要跨市場比較因子 signal 的強度，MSCI 的 country-specific mean + global std 是業界公認的標準化方式。但 v3 Gate 的 pass/fail 設計天然迴避了這個問題——比較的是 Gate status（通過/未通過），不是 signal 的絕對值。
...
- **在本框架的應用：** Gate 1 P1 改進的理論依據——從固定門檻 2.0 升級為 BHY 連續校正時，本文提供了為什麼 BHY 比離散階梯門檻（2.0/2.5/3.0）更原則性的論證。未來若需更精確的校正，可進一步採用本文的 Bayesian framework。
...
- **在本框架的應用：** Gate 1 的 OR 條件（IC_IR OR Q1-Q5 spread）等效於兩次檢定。P1 可使用 Romano-Wolf stepdown correction 處理此 OR 帶來的多重檢定膨脹，比 Bonferroni 近似（門檻從 2.0 提高至 ~2.24）更精確。
...
- **在本框架的應用：** 支持 Net_Spread（`Q1-Q5 Spread - 2 × cost × Turnover`）作為 Profile 指標——雖然不作為 Gate（成本是 implementation 問題），但讓使用者快速判斷扣除成本後的殘餘 alpha。本文的核心洞見是 Breakeven Cost 和 Gross Spread 分開報告不足以捕捉交互效應。
