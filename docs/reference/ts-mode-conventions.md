...
`Common × Continuous` metrics (`common_beta`, `common_quantile`, `common_asymmetry`
...
per-asset time-series regressions** — run inside `compute_common_betas` —
...
`common_beta` is a two-stage estimator: stage 1 is a per-asset OLS of
...
belongs in the `common_beta` family and should be interpreted with the stage-1
plain-SE caveat here.

## No persistence diagnostic on this family

The `common_beta` family emits **no** unit-root / ADF persistence
...
The `common_beta` family inverts this: the per-asset stage-1 regression
...
- **Overlap structure**: `h ≠ signal_horizon` produces an MA(h−1)
  ...
  this for HAC-using metrics; the plain-SE stage-1 in `common_beta` does
  not.
