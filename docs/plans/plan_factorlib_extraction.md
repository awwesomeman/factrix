# Plan: Extract factorlib to Standalone Repo

**Date**: 2026-04-20
**Status**: Proposed — pending execution
**Driver**: factorlib 已達 v3.3.0、533 tests 全綠、無未決 refactor——是自然切點。
...
1. 把 `factorlib/` 拆成**獨立 repo**，具備套件該有的完整形態（src + tests + pyproject + README + LICENSE + CI + CHANGELOG）。
2. 本 workspace 轉型為 **factor-research consumer**：保留研究資料、下游規格（個股評分等）、研究 notebook，透過 **git submodule + editable install** 使用 factorlib。
3. 保留**動態更新能力**：改 factorlib 立即生效；同時讓 workspace 的每個 commit 都**記錄當下使用的 factorlib SHA**（可重現）。
...
- **研究資料版本化（Data Versioning）**：`sample_data.parquet`、`tw_stock_daily_*.parquet`、`mlflow.db`、`mlruns/` 目前都在 `.gitignore`，workspace 沒有資料版本。6 個月後要重現某個 2026-04-20 的研究結果：factorlib SHA 有解（submodule），**資料沒解**。本計畫不處理，但完成後應盡快評估 DVC / Git LFS / 外部 S3+hash 三選一。（quant 側硬洞）
- **Notebook stored outputs 生命週期**：factorlib BC change 會讓 stored output 過時（T8 demo execution 未解、上週 `q1_q5_spread → long_short_spread` 重現）。本計畫遷移後仍存在。workspace 應加 nbstripout pre-commit hook 或明定「notebook 結果以最新 `bump factorlib SHA` 重跑為準」的 convention。
...
└── factor-analysis/                   # 本 workspace（繼續存在，改用 submodule）
    ...
    ├── pyproject.toml                 # 改 name，把 factorlib 設成 local editable dep
    ├── .gitignore
    ├── scripts/check_submodule.sh     # fail-fast 檢查 submodule 已 init
    ├── external/
    │   └── factorlib/                 # git submodule，指向 factorlib repo 某個 SHA
...
| 類別 | 路徑 | 去向 | 備註 |
|------|------|------|------|
| 套件原始碼 | `factorlib/**` | **factorlib repo** | 整包搬 |
| 測試 | `tests/**` | **factorlib repo** | 包含 profiles/、stats/ 子目錄 |
| 打包 | `pyproject.toml`, `uv.lock`, `.gitignore`, `README.md` | **factorlib repo** | pyproject 改 name；uv.lock 在 Phase 2 regenerated |
| Library demo | `experiments/demo.ipynb`, `experiments/_build_demo.py`, `experiments/benchmark_*.py`, `experiments/dogfood_with_extra_columns.py` | **factorlib repo** → `examples/` | 示範套件 API 用 |
...
| 待裁決 | `docs/factor_screening*.md`, `docs/factor_screening_gemini.md` | **pending** | Phase 1 必須決策：如果是 factorlib 內建 screening feature 規劃 → factorlib；如果是 workspace 研究流程 → workspace |
| 研究資料 | `*.parquet`, `mlflow.db`, `mlruns/` | workspace | 已 gitignored；版本化是 follow-up |

**邊界裁決**：
- `docs/naming_convention.md` 屬 API contract 規則，但仍留 workspace。若 factorlib 日後公開，將核心規則**抄寫**到 `factorlib/README.md` 的 "Naming Rules" 區塊或新的 `ARCHITECTURE.md`，不搬原文件。
...
1. 處理當前 M 狀態檔：
   - `factorlib/_api.py`, `factorlib/charts/quantile.py`, `factorlib/config.py`, 多個 `factorlib/metrics/*.py`, `tests/conftest.py`, `tests/test_factor_session.py`——commit 或 stash 完
...
```bash
grep -rn "sample_data\|tw_stock_daily\|\.parquet" tests/ factorlib/ \
...
- [ ] 逐行確認歸屬表
- [ ] **裁決 `docs/factor_screening*.md`**：是 factorlib 內建 feature 規劃還是 workspace 研究流程？
- [ ] 確認 `docs/naming_convention.md` 留 workspace（且同意 Phase 3 寫入 factorlib README / ARCHITECTURE）
- [ ] 確認 `docs/study/` 追蹤策略
...
```bash
cd ~/Desktop/dst/code
git clone factor-analysis/ factorlib-extract/
cd factorlib-extract/
...
```bash
cp .git/filter-repo/commit-map ~/Desktop/dst/code/factorlib-sha-map.txt
```

格式：每行 `<old_sha> <new_sha>`。Phase 4 用它批改舊引用。

### P2.3 結構調整（在 factorlib-extract/ 內）

```bash
# 1. experiments/ → examples/
git mv experiments examples

# 2. factorlib/README.md 拉到根當主 README
git mv factorlib/README.md README.md

# 3. pyproject.toml 改 name: "factor-analysis" → "factorlib"
# （手改 pyproject.toml）

git add -A
git commit -m "chore: rename package factor-analysis→factorlib, restructure examples/"
```
...
uv run python -c "import factorlib as fl; print(fl.__version__)"
```

**退回機制**：Phase 2 全程在 `factorlib-extract/` 獨立目錄；失敗 `rm -rf factorlib-extract/` 從 `pre-extraction-backup` tag 重 clone 重做。
...
1. GitHub 建立空 repo：`<you>/factorlib`
2. 推送：
   ```bash
   cd factorlib-extract/
   git remote remove origin
   git remote add origin git@github.com:<you>/factorlib.git
...
factorlib 的 BC change 對下游 research 殺傷力大（例：最近 `q1_q5_spread → long_short_spread`），SemVer 與 CHANGELOG 是 extraction 的必要基礎設施。

### P3.4 **必要**：ARCHITECTURE.md（3 頁 current-state snapshot）

factorlib repo 若完全沒有 docs/，cold reader（含 AI agent）對設計一頭霧水。寫一份**現況描述**（不是搬歷史）：
...
```bash
mv ~/Desktop/dst/code/factorlib-extract ~/Desktop/dst/code/factorlib
```
...
### P4.1 加 submodule + 寫新 pyproject（不刪舊 factorlib/）

```bash
cd ~/Desktop/dst/code/factor-analysis/

git submodule add git@github.com:<you>/factorlib.git external/factorlib
...
version = "0.1.0"
description = "Factor research workspace consuming factorlib"
...
[tool.uv.sources]
factorlib = { path = "external/factorlib", editable = true }
...
```bash
#!/usr/bin/env bash
if [ ! -f external/factorlib/pyproject.toml ]; then
  echo "ERROR: external/factorlib is empty. Run: git submodule update --init --recursive" >&2
  exit 1
fi
```
...
- `external/factorlib/` 指向套件本體（link to factorlib repo）
...
uv run python -c "import factorlib as fl; print(fl.__version__, fl.__file__)"
# 確認 fl.__file__ 指到 external/factorlib/factorlib/__init__.py
```

挑一個最常用的 research notebook 或 script 跑過——確認 editable import 成功、沒有 import path shadowing（top-level `factorlib/` 還在所以可能有衝突，下一步就清掉）。

### P4.3 刪除已遷出的檔案

```bash
git rm -r factorlib/ tests/
...
```bash
uv sync
uv run python -c "import factorlib as fl; print(fl.__file__)"
# 確認仍指到 external/factorlib/，沒有因為頂層 factorlib/ 消失而壞掉
```

### P4.4 批改舊 SHA 引用

用 P2.2 保留的 `factorlib-sha-map.txt` 批改 workspace 內的舊 SHA：
...
done < ~/Desktop/dst/code/factorlib-sha-map.txt
...
```bash
git add .
git commit -m "refactor(workspace): extract factorlib to submodule, rewrite pyproject/README"
```
...
### P5.2 **mlflow 自動 tag factorlib SHA**（研究可重現性必備）
...
def tag_factorlib_sha():
    """Log current factorlib commit SHA so runs can be traced to library version."""
    try:
        sha = subprocess.check_output(
            ["git", "-C", "external/factorlib", "rev-parse", "HEAD"],
            stderr=subprocess.DEVNULL,
        ).decode().strip()
        mlflow.set_tag("factorlib_sha", sha)
        mlflow.set_tag("factorlib_version", __import__("factorlib").__version__)
    except (subprocess.CalledProcessError, FileNotFoundError):
        mlflow.set_tag("factorlib_sha", "unknown")
...
沒這個 tag，`mlruns/` 裡的 experiment 無法對應回 factorlib 版本，code 可重現但結果來源不明。
...
# ... edit factorlib/_api.py ...
uv run pytest                          # factorlib 自己的 test suite 全綠
git commit -am "feat: ..."
git push

# 回到 workspace，bump submodule pointer
cd ../..
git add external/factorlib             # 記錄新的 SHA
git commit -m "chore: bump factorlib to <short-sha>: <why>"
```

### P5.4 驗證清單

- [ ] `import factorlib` 成功，`fl.__file__` 指到 `external/factorlib/factorlib/__init__.py`
- [ ] editable install：改 factorlib 源碼後 workspace 立即看到（Jupyter 需 restart kernel）
- [ ] `external/factorlib` 內 `uv run pytest` 533 tests 全綠
- [ ] factorlib repo 的 GitHub Actions CI 綠燈
- [ ] workspace 挑 1-2 個最常用 research notebook/script 能正常執行
- [ ] workspace `git log` 顯示 submodule bump commit 帶有 factorlib SHA
- [ ] fresh clone `--recursive` + `uv sync` 成功
- [ ] `scripts/check_submodule.sh` 在未 init 情境給出明確錯誤
- [ ] mlflow run tag 能讀到 factorlib_sha
...
| pyproject rename 後 import 異常 | 套件 load 失敗 | Phase 2.4 重建 uv.lock | P2.5 `import factorlib` |
| **filter-repo 重寫 SHA** | 舊 SHA 引用全失效 | P2.2 保留 commit-map，P4.4 批改舊引用 | P4.4 後 grep 舊 SHA 應為 0 |
| workspace notebook 找不到 factorlib | 研究流程中斷 | P4.2 先驗再刪 + Phase 5 fresh clone 驗證 | P5.1 |
| submodule UX 失敗 | 新機器裝不起來 | `scripts/check_submodule.sh` + README 粗體警示 | P5.1 模擬無 `--recursive` |
| factorlib BC 打壞 workspace | 研究結果不可重現 | submodule SHA pin + SemVer + CHANGELOG | 正常 dev cycle |
| mlflow run 無法對應 factorlib 版本 | 研究可重現性半壞 | P5.2 自動 tag factorlib_sha | mlflow UI 看 run tag |
...
**全案退回**：Phase 2/3 失敗 → 刪 `factorlib-extract/`、從 `pre-extraction-backup` tag 重 clone。Phase 4 失敗 → `git reset --hard` 回 Phase 4 前、`git submodule deinit external/factorlib`（舊 factorlib/ 還在所以可直接用）。
