...
本文檔總結了從「資深量化研究員」與「後端工程師」的雙重視角，針對 `factorlib` 套件在**「大規模因子探索/篩選」**以及**「開源套件架構」**上的最佳設計實踐。

---

## 1. 架構核心原則：計算與基礎設施分離

當 `factorlib` 作為一個讓外部使用者或跨部門分析師安裝的套件時，「邊界（Scope）控制」是確保套件容易被採用且能無限擴展的關鍵。

> [!IMPORTANT]
> **混合模式（Hybrid Approach）**
> 核心庫 (`factorlib`) 必須保持純粹的無狀態 (Stateless)，針對 DB 與儀表板採用 **Batteries Included but Swappable (內建輕量預設，但可抽換)** 的策略。這能讓新使用者零門檻上手，同時滿足企業級的擴展需求。
...
- **可互動 Web UI (層次二 - 可選安裝)**：將儀表板作為 Optional Dependency (`pip install factorlib[dashboard]`)。提供基於 Streamlit 或 Dash 的 CLI 命令（如：`factorlib server --db=sqlite:///factors.db`），使用者可以立刻獲得類似 MLflow 的精美視覺化。
...
- **機制**：`factorlib` 負責將 DSL 安全地解析為底層的 `Polars` 運算式。
...
為實現上述功能，`factorlib` 專案目錄將可按以下結構進行模組化切割：
...
└── ui/                    # 獨立的儀表板模組 (可選依賴)
    ├── cli.py             # CLI 啟動點 (factorlib server)
