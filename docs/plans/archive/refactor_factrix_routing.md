...
> 背景：現有 factrix 僅支援截面風格因子（個股選股）。擴展到事件訊號、宏觀 panel、
...
| `metrics/` | 獨立可調用的指標，每個回傳 `MetricOutput` |
| `factors/` | 因子生成器 |
| `charts/` | 圖表（optional: `pip install factrix[charts]`） |
| `integrations/mlflow.py` | MLflow tracking（optional: `pip install factrix[mlflow]`） |
| `integrations/streamlit/` | Dashboard app（optional: `pip install factrix[dashboard]`） |
...
```bash
pip install factrix              # 核心
pip install factrix[charts]      # + plotly
pip install factrix[mlflow]      # + mlflow tracking
pip install factrix[all]         # 全裝
```
...
try:
    ...
    raise ImportError(
        "Charts require plotly. Install with: pip install factrix[charts]"
    )
```
...
| `from factrix.tools import ...` | `from factrix.metrics import ...` | metrics 更精確 |
| `FactorTracker` | `from factrix.integrations.mlflow import FactorTracker` | optional dep |
...
| Persona | 使用場景 | 程式碼 |
|---------|---------|--------|
| Quick Screener | 快速篩選 | `import factrix as fl` → `fl.quick_check(df, "X")` |
...
| Factor Developer | 單一指標 | `from factrix.metrics import compute_ic, ic_ir` |
| Factor Developer | 圖表 | `from factrix.charts import report_charts`（需 `[charts]`） |
| Factor Developer | 追蹤 | `from factrix.integrations.mlflow import FactorTracker`（需 `[mlflow]`） |
| Portfolio Builder | 因子篩選 | `fl.batch_evaluate()` → `fl.compare()` + `from factrix.metrics import spanning_alpha` |
...
```python
from factrix.metrics import compute_ic, quantile_spread, spanning_alpha
```
...
| `gates/config.py` | `factrix/config.py` |
...
1. `pytest tests/ -v` 全綠
2. `from factrix.metrics import compute_ic, quantile_spread` works
...
12. `from factrix._types import MetricOutput` works
13. `from factrix._stats import bhy_threshold` works
14. `from factrix.charts import report_charts` 未裝 plotly 時報明確 ImportError
15. `from factrix.integrations.mlflow import FactorTracker` 未裝 mlflow 時報明確 ImportError
