...
- `factrix/evaluation/_protocol.py` — `Artifacts` 只有 `prepared` /
  `intermediates` / `config` / `factor_name`，**沒有** per-metric MetricOutput 存放處
- `factrix/evaluation/profiles/cross_sectional.py::from_artifacts` L126-254
...
- `factrix/metrics/ic.py::regime_ic` 的 MetricOutput 在 `_augment_level2_intermediates`
...
- `factrix/_types.py::MetricOutput` 是 `@dataclass`（非 frozen）；`metadata: dict`
  是 mutable
...
3. 新模組 `factrix/reporting.py`：
   ...
   - **0.3d**
4. `factrix/__init__.py` export + README + demo.ipynb 新 section
...
- **`from_artifacts` signature 改 tuple return 是 breaking**（§3.4 option b）：
  ...
  **緩解**：grep 範圍限於 `factrix/_api.py` (call site) + 4 個 Profile class
...
- [ ] Owner 確認 §3.7 單一 `describe_profile_values` + auto-discover detail
      （建議 (a)）
- [ ] Owner 確認 detail view 實作放 `factrix/reporting.py` 新模組
      （非 `_api.py`）
