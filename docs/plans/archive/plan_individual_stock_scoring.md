...
> **目標：** 每個模組可 `from factrix.tools.xxx import ...` 獨立使用，附帶可獨立跑的 example/test。
...
from factrix.gates.presets import CROSS_SECTIONAL_GATES
from factrix.gates.significance import significance_gate
from factrix.gates.oos_persistence import oos_persistence_gate
from factrix.gates.pipeline import evaluate_factor
...
交付物：`from factrix.preprocess.returns import compute_forward_return` 可獨立使用。
