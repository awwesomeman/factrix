...
把 factrix 的研究核心抽象從 Gate（pass/fail 決策鏈）換成 `FactorProfile`（per-type typed 剖面）。ADR 已拍板 A–G 七個抉擇、collapse 至 2 個 top-level entry point、無相容層。
...
- `factrix/stats/multiple_testing.py` BHY module
...
### A.1 核心型別與 protocol
- 新增 `factrix/_types.py` 加入 `PValue = NewType("PValue", float)`、`Verdict = Literal["PASS", "FAILED"]`、`Diagnostic` dataclass。
- 新增 `factrix/evaluation/profiles/_base.py`：
...
- `from_artifacts(cls, artifacts) -> Self` classmethod：呼叫既有 `factrix.metrics.*` 函數組裝
...
### A.4 ProfileSet
- 新增 `factrix/evaluation/profile_set.py`
...
- 新增 `factrix/stats/multiple_testing.py`:
  ```python
...
### A.6 Diagnose rules
- 新增 `factrix/evaluation/diagnostics/_rules.py`
...
- 新增 `factrix/metrics/redundancy.py`：
  ```python
...
- export 於 `factrix.__init__`：`fl.redundancy_matrix`
...
### B.1 刪除目錄 / 檔案 ✅
- `factrix/evaluation/gates/`（整包）
- `factrix/evaluation/presets.py`
- `factrix/evaluation/_caution.py`（邏輯已在 A.6 搬進 diagnostics/_rules.py）
- `tests/test_gates.py`、`tests/test_*_gate.py`
- 舊的 `test_p1_additions.py` 中的 bhy_threshold 測試（已搬至 `tests/stats/`）

### B.2 刪除型別與 API ✅
- `factrix/evaluation/_protocol.py`：
...
- `factrix/evaluation/pipeline.py`：
...
| 檔案 | Phase | 動作 |
|------|-------|------|
| `factrix/_types.py` | A.1 | 新增 PValue/Verdict/Diagnostic |
| `factrix/evaluation/profiles/_base.py` | A.1 | 新增（Protocol + register_profile + registry） |
| `factrix/evaluation/profiles/cross_sectional.py` | A.2 | 新增 |
| `factrix/evaluation/profiles/event.py` | A.2 | 新增 |
| `factrix/evaluation/profiles/macro_panel.py` | A.2 | 新增 |
| `factrix/evaluation/profiles/macro_common.py` | A.2 | 新增 |
| `factrix/evaluation/profiles/__init__.py` | A.2 | 新增（import 觸發 decorator） |
| `factrix/evaluation/_protocol.py` | A.3 → B.2 | 加 Artifacts.from_raw/compact → 刪 Gate/EvaluationResult/舊 FactorProfile |
| `factrix/evaluation/profile_set.py` | A.4 | 新增 |
| `factrix/stats/__init__.py` | A.5 | 新增 |
| `factrix/stats/multiple_testing.py` | A.5 | 新增 |
| `factrix/evaluation/diagnostics/_rules.py` | A.6 | 新增 |
| `factrix/__init__.py` | A.7 → B.3 | 加 evaluate/evaluate_batch/list_factor_types/redundancy_matrix（雙 export）→ 清舊 exports |
| `factrix/_api.py` | A.8 → B.2 | describe_profile 換內臟 → 刪 quick_check/batch_evaluate/compare/metric dicts |
| `factrix/metrics/redundancy.py` | A.9 | 新增（redundancy_matrix） |
| `factrix/_stats.py` | B.2 | 刪 bhy_threshold |
| `factrix/evaluation/pipeline.py` | B.2 | 刪除 gate-era `evaluate()`，保留 `build_artifacts()` + per-type helper |
| `factrix/evaluation/profile.py` | B.1 | 刪除（`compute_profile` 被 Profile.from_artifacts 取代） |
| `factrix/evaluation/gates/` | B.1 | 刪除目錄 |
| `factrix/evaluation/presets.py` | B.1 | 刪除 |
| `factrix/evaluation/_caution.py` | B.1 | 刪除 |
| `factrix/charts/__init__.py` | B.2 | `report_charts(result)` → `report_charts(artifacts)` |
| `factrix/integrations/mlflow.py` | B.2 | `log_evaluation(result)` → `log_profile(profile)` |
...
| `factrix.evaluation.gates.*` | 整包刪除 |
| `factrix.evaluation.presets.default_gates_for` | 刪除 |
| `factrix.evaluation._caution.check_caution` | 由 `profile.diagnose()` → `list[Diagnostic]` 取代 |
| `factrix.evaluation.profile.compute_profile` | 刪除（每個 Profile 有自己的 `from_artifacts`） |
| `factrix.evaluation.pipeline.evaluate(...)`（gate-era） | 移至 `factrix._api.evaluate`（profile-era dispatch） |
| `factrix._stats.bhy_threshold(t_stats)` | `factrix.stats.multiple_testing.bhy_adjust(p_values)` / `ProfileSet.multiple_testing_correct` |
...
Phase A 完成後 factrix 公開 API 即包含篩選計畫的全部能力；不需要另外 follow-up。
