...
- `factrix/preprocess.py`（或 `compute_forward_return`）— 加 `/= forward_periods`
- `quantile_spread()`, `quantile_spread_vw()` — 移除 `_annualize_return()` 呼叫，value 改為 per-period
- `factrix/tools/_helpers.py` — `_annualize_return()` 移至 dashboard 呈現層或刪除
...
- `factrix/gates/profile.py` — profitability 從 6 個指標減為 5 個
...
- `factrix/tools/regression/orthogonalize.py` — 加 beta + R² 回傳
- `factrix/tools/regression/spanning.py` — `spanning_alpha` 加 beta + R² 回傳
...
- `factrix/tools/_typing.py` — 欄位改名 + 新增 metadata 標準 key 文件
- `factrix/tools/series/significance.py` — `_significance_marker()` 改吃 p_value
...
- `factrix/tools/panel/ic.py` — `regime_ic()` 和 `multi_horizon_ic()` 的 value/stat 計算調整
