...
> 優先序：P4
> 設計依據：`refactor_factorlib_routing.md` §2–§3（指標對照表 + 統計備註 §3.4–§3.6）
...
屬於因子有效性分析，在 factorlib 範圍。Portfolio-level 策略指標
...
14. `from factorlib.metrics import corrado_rank_test, compute_mfe_mae` 可獨立使用
...
factorlib 的 per-event MAE p95 已提供尾部風險資訊，不需要 portfolio
層面的 drawdown。
