...
`fl.evaluate()` 維持 production 單次入口；`factorlib.metrics.*` 降級為
...
- IDE autocomplete 沒法列出「CS 因子可用的 metric」——`from factorlib.metrics import ...` 是 flat 命名空間
...
`factorlib.metrics.*` 的 signature；`fl.factor()` 是**純加層**。已存在的
code path 全部 bit-for-bit 不動（迴歸風險接近 0）。

## 2. 現況（source-code 驗證）

- `factorlib/_api.py::evaluate` L314 —— 目前唯一的高階入口，一次呼叫回
  `FactorProfile`，內部走 `_evaluate_one` → `build_artifacts` →
  `profile_cls.from_artifacts`
- `factorlib/evaluation/pipeline.py::build_artifacts` L27 —— public top-level
  export（`factorlib/__init__.py:39`），接受 `(df, config)`，回 `Artifacts`
- `factorlib/evaluation/_protocol.py::Artifacts` L67 —— 已經是完整的 cache
  bundle：`prepared` / `intermediates` / `metric_outputs` / `config` /
  `factor_name`
- `factorlib/metrics/ic.py::ic` L58 —— 接 `ic_df`（processed）
- `factorlib/metrics/quantile.py::quantile_spread` L86 —— 接 `df`（prepared
  panel），含 `_precomputed_series` 私有 kwarg（library-internal 優化）
- `factorlib/metrics/__init__.py` —— 23 個 metric function 一次 re-export，
...
- 缺點：factorlib 現有 top-level API 沒有 PascalCase constructor 風格，
...
然後 call 既有 `factorlib.metrics.*` primitive**。**建議**。
```python
class CrossSectionalFactor(Factor):
    def ic(self, forward_periods: int | None = None) -> MetricOutput:
...
### 3.13 `factorlib.metrics.*` 統一回 `MetricOutput`（single return contract）

**背景**：審計 `factorlib/metrics/*.py` 顯示 29 個 metric 已回 `MetricOutput`，
但 4 個**偏離 contract**：
...
- **不改 `factorlib.metrics.*` primitive signature**——保留作為 library
...
1. **PR 1 — L1 cache pre-populate guard（§3.12）**：`cross_sectional.py`
   ...
   ——**0.2d**
2. **`factorlib/factor.py` 新模組**：`Factor` base + `CrossSectionalFactor`
   subclass + `_FACTOR_REGISTRY`。Factor method 逐一 wire up 到
   `factorlib.metrics.*` primitive，含 §3.4 per-call override +
   §3.6 cache 讀寫邏輯 + §3.7 `__post_init__` 驗證 +
   §3.10 cross-type AttributeError message。
   ——**0.4d**
3. **`factorlib/_api.py::factor` factory function**：`fl.factor(...)`
   寫在 `_api.py`（跟 `evaluate` 放同檔），簽名鏡像 `evaluate`（§3.8），
   dispatch 到 `_FACTOR_REGISTRY`。Top-level export
   (`factorlib/__init__.py`) 加 `factor` + `Factor` / `CrossSectionalFactor`
   type hint。——**0.1d**
...
6. **Docs**：README 加 "Research workflow" section 對比
   ...
   `factorlib.metrics.*` docstring 加 "low-level primitive — for research
   use `fl.factor()`"。——**0.2d**
