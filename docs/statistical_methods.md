...
factorlib 的每個指標都對應業界 / 學界認可的方法。本文列出所有**實際採用**的方法、**經評估但刻意未實作**的部分、以及完整文獻清單，方便你把結果帶進研究或報告時能明確交代技術背景。

> 本文職責：metric 的**論文依據**與**採用取捨**（含與論文的 deviation）。**公式細節**不在這裡 — 請讀對應 `.py` module 的 docstring（`help(factorlib.metrics.<name>)`）。文件整體分工見 [README.md](../README.md)。
...
精確實作：`factorlib/metrics/ic.py::compute_ic` / `ic_mean`（rank method、per-date N 過濾規則、IR 公式）
...
精確實作：`factorlib/metrics/ic.py::ic` + `factorlib/metrics/_helpers.py::_sample_non_overlapping`
...
精確實作：`factorlib/_stats.py::_newey_west_se` / `::_newey_west_t_test`（Bartlett kernel + lag rule）；`factorlib/metrics/ic.py::ic_newey_west`（IC 特化 call site）
...
精確實作：`factorlib/metrics/quantile.py`（spread / VW spread / group returns）、`factorlib/metrics/monotonicity.py`、`factorlib/metrics/concentration.py`（HHI⁻¹ effective-n）
...
精確實作：`factorlib/metrics/caar.py::compute_caar` + `::caar`（signed_car 公式、非重疊採樣、t-stat）
...
精確實作：`factorlib/metrics/caar.py::bmp_test`（SAR 標準化、跨事件 std、常態近似 z）
...
精確實作：`factorlib/metrics/corrado.py::corrado_rank_test`（pooled-rank 簡化版，與論文原式的 deviation 見 docstring）
...
精確實作：`factorlib/metrics/clustering.py::clustering_diagnostic`（時間維度 HHI + effective-n 歸一化）
...
精確實作：`factorlib/metrics/event_quality.py::event_hit_rate` / `::profit_factor` / `::event_skewness` / `::event_ic`
...
精確實作：`factorlib/metrics/fama_macbeth.py::compute_fm_betas`（stage-1 per-date OLS）+ `::fama_macbeth`（stage-2 NW t-test on λ）；`::pooled_ols` 作為 robustness 對照
...
精確實作：`factorlib/metrics/ts_beta.py::compute_ts_betas`（per-asset OLS）+ `::ts_beta`（跨資產 t-test）+ `::ts_beta_single_asset_fallback`（N=1 退化路徑）
...
精確實作：`factorlib/_stats.py::_adf` + `::_adf_pvalue_interp`（MacKinnon constant-only 插值、p 上限 0.95 clip）
...
精確實作：`factorlib/stats/multiple_testing.py::bhy_adjust` / `::bhy_adjusted_p`（c(m) scaling、right-cummin 單調化、two-stage `n_total` 支援）
...
精確實作：`factorlib/preprocess/normalize.py::mad_winsorize`（k 預設值、per-date 套用、consistency constant）
...
精確實作：`factorlib/preprocess/normalize.py` + `factorlib/preprocess/pipeline.py`（step 5 per-date z-score）
...
以下方法**在設計討論中有考慮**，但因增量收益有限或會偏離 lean-dependency 原則而未納入。若你的 workflow 真的需要，建議使用 `statsmodels` / `arch` / `linearmodels` 等專門套件在 factorlib 輸出之外處理。
