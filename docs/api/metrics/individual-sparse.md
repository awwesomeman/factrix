...
| `{0, R}` event magnitude (`R` may be signed) | `caar` | `abnormal_return * factor`; magnitude is preserved. `bmp_z` uses the event sign only. |
| Does event size rank realized event payoff? | `event_ic` | Spearman correlation between `abs(factor)` and `signed_car`, where `signed_car = abnormal_return * sign(factor)`. |
