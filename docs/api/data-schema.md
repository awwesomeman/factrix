...
| `asset_id` | `Utf8` / `Categorical` | Cross-section identifier. Identical for COMMON-scope factors. Scope detection reads **finite cells only**, so a missing cell does not make a broadcast factor individual (`df.filter(pl.col("factor").cast(pl.Float64).is_finite()).group_by("date").agg((pl.col("factor").n_unique() == 1).alias("is_common"))["is_common"].all()` is `True`). |
