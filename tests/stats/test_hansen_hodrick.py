...
class TestHansenHodrickSe:
    def test_h_one_collapses_to_iid(self):
        ...
        se, clamped = _hansen_hodrick_se(x, overlap_periods=1)
    ...
    def test_positive_autocorrelation_inflates_se(self):
        ...
        se_iid, _ = _hansen_hodrick_se(x, overlap_periods=1)
        se_overlap, clamped = _hansen_hodrick_se(x, overlap_periods=12)
        assert clamped is False
    ...
    def test_negative_variance_clamps_and_flags(self):
        ...
        se, clamped = _hansen_hodrick_se(x, overlap_periods=2)
        assert clamped is True
        assert se == 0.0

    def test_short_sample_returns_zero(self):
        se, clamped = _hansen_hodrick_se(np.array([1.0]), overlap_periods=4)
        assert se == 0.0
        assert clamped is False

    def test_lag_capped_at_n_minus_one(self):
        # overlap_periods exceeds n: lag should clip to n-1, math stays finite.
        x = np.array([1.0, 2.0, 3.0, 4.0])
        se, clamped = _hansen_hodrick_se(x, overlap_periods=10)
        assert np.isfinite(se)
        # Same x with overlap_periods=4 hits the same effective lag (n-1=3).
        se_at_cap, _ = _hansen_hodrick_se(x, overlap_periods=4)
...
class TestHansenHodrickTTest:
    def test_h_one_matches_iid_t(self):
        ...
        t, _, _, clamped = _hansen_hodrick_t_test(x, overlap_periods=1)
        assert clamped is False
        ...
        se_hh, _ = _hansen_hodrick_se(x, overlap_periods=1)
    ...
    def test_clamped_returns_not_computable(self):
        ...
        x = np.array([1.0, -1.0] * 50)
        t, p, marker, clamped = _hansen_hodrick_t_test(x, overlap_periods=2)
        assert clamped is True
        assert math.isnan(t)
        assert math.isnan(p)
        assert marker == ""
    ...
    def test_short_sample(self):
        t, p, marker, clamped = _hansen_hodrick_t_test(
            np.array([1.0, 2.0]), overlap_periods=2
        )
        assert math.isnan(t)
        assert math.isnan(p)
    ...
    def test_overlap_inflates_se_lowers_t(self):
        ...
        t_iid, _, _, _ = _hansen_hodrick_t_test(x, overlap_periods=1)
        t_overlap, _, _, _ = _hansen_hodrick_t_test(x, overlap_periods=12)
...
def test_hansen_hodrick_rejects_non_finite():
    ...
    with pytest.raises(ValueError, match="finite"):
        _hansen_hodrick_se(np.array([0.1, float("nan"), 0.2]), overlap_periods=2)
    with pytest.raises(ValueError, match="finite"):
        _hansen_hodrick_t_test(
            np.array([0.1, float("nan"), 0.2, 0.3]), overlap_periods=2
        )
