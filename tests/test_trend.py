...
class TestTheilSenSlope:
    def test_perfect_uptrend(self):
        ...
        result = ic_trend(_make_series(values), forward_periods=1)
    ...
    def test_adf_flags_random_walk(self):
        ...
        result = ic_trend(_make_series(walk), forward_periods=1)
    ...
    def test_adf_clears_stationary_noise(self):
        ...
        result = ic_trend(_make_series(noise), forward_periods=1)
        assert result.metadata["unit_root_suspected"] is False

    def test_adf_threshold_none_disables_check(self):
        values = [0.01 * i for i in range(20)]
        result = ic_trend(_make_series(values), forward_periods=1, adf_threshold=None)
    ...
    def test_adf_threshold_custom_value(self):
        ...
        noise = rng.standard_normal(200).tolist()
        strict = ic_trend(_make_series(noise), forward_periods=1, adf_threshold=0.01)
        permissive = ic_trend(
            _make_series(noise), forward_periods=1, adf_threshold=0.50
        )
    ...
    def test_adf_threshold_out_of_range_raises(self):
        ...
        with pytest.raises(ValueError, match="adf_threshold"):
            ic_trend(_make_series(values), forward_periods=1, adf_threshold=1.5)
        with pytest.raises(ValueError, match="adf_threshold"):
            ic_trend(_make_series(values), forward_periods=1, adf_threshold=0.0)
    ...
    def test_all_nan_input_short_circuits(self):
        ...
        result = ic_trend(_make_series([math.nan] * 30), forward_periods=1)
        assert math.isnan(result.value)
...
class TestPerfectAndFlatSeries:
    ...
    def test_perfect_line_is_decisive(self):
        result = ic_trend(
            _make_series([0.01 * i for i in range(20)]), forward_periods=1
        )
        assert result.metadata["ci_excludes_zero"] is True
    ...
    def test_flat_series_withholds_the_test(self):
        ...
        result = ic_trend(_make_series([0.05] * 20), forward_periods=1)
...
class TestMannKendallReference:
    def test_stat_is_kendall_tau_on_the_index(self):
        ...
        result = ic_trend(_make_series(vals), forward_periods=1, adf_threshold=None)
...
class TestOverlapCorrection:
    ...
    def test_strides_before_testing(self):
        ...
        result = ic_trend(_make_series(values), forward_periods=5, adf_threshold=None)
    ...
    def test_raw_date_count_gates_against_the_scaled_floor(self):
        ...
        result = ic_trend(_make_series(values), forward_periods=5)
        assert math.isnan(result.value)
    ...
    def test_null_size_on_an_overlapping_series(self):
        ...
        for _ in range(n_reps):
            ...
            p = ic_trend(
                _make_series(vals.tolist()), forward_periods=5, adf_threshold=None
            ).p_value
            rejects += p < 0.05
    ...
    def test_residual_persistence_is_flagged(self):
        ...
        result = ic_trend(
            _make_series(x.tolist()), forward_periods=1, adf_threshold=None
        )
    ...
    def test_unit_root_flag_now_carries_a_warning_code(self):
        ...
        result = ic_trend(_make_series(walk.tolist()), forward_periods=1)
        assert result.metadata["unit_root_suspected"] is True
