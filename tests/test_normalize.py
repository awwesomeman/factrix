...
class TestMADWinsorize:
    def test_clips_outlier(self):
        df = pl.DataFrame({
            ...
            "factor": [1.0, 2.0, 3.0, 4.0, 100.0],  # 100 is extreme
        }).with_columns(pl.col("datetime").cast(pl.Datetime("ms")))
        result = mad_winsorize(df, "datetime", "factor", n_mad=3.0)
    ...
    def test_noop_when_disabled(self):
        ...
        result = mad_winsorize(df, "datetime", "factor", n_mad=0)
        assert result["factor"].to_list() == df["factor"].to_list()

    def test_per_date(self):
        df = pl.DataFrame({
            "datetime": [datetime(2024, 1, 1)] * 3 + [datetime(2024, 1, 2)] * 3,
            "factor": [1.0, 2.0, 100.0, 10.0, 20.0, 1000.0],
        }).with_columns(pl.col("datetime").cast(pl.Datetime("ms")))
        result = mad_winsorize(df, "datetime", "factor", n_mad=3.0)
        # Both dates should clip their outlier independently
        d1 = result.filter(pl.col("datetime") == datetime(2024, 1, 1))["factor"].max()
        d2 = result.filter(pl.col("datetime") == datetime(2024, 1, 2))["factor"].max()
        assert d1 < 100.0
        assert d2 < 1000.0
...
class TestCrossSectionalZScore:
    def test_zero_mad(self):
        ...
        result = cross_sectional_zscore(df, "datetime", "factor")
