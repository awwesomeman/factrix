...
def test_multiple_testing_correct_emits_info(cs_profiles_and_artifacts, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="factorlib.evaluation"):
        ProfileSet(profiles).multiple_testing_correct(fdr=0.05)
    msgs = [r.message for r in caplog.records if r.name == "factorlib.evaluation"]
...
def test_pass_with_warnings_emits_warning(cs_profile_strong, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="factorlib.evaluation"):
...
def test_sample_non_overlapping_debug_and_warning(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="factorlib.metrics"):
        _sample_non_overlapping(df, forward_periods=5)
    records = [r for r in caplog.records if r.name == "factorlib.metrics"]
...
def test_newey_west_warns_when_lags_exceed_sample_ratio(caplog):
    import numpy as np
    from factorlib._stats import _newey_west_t_test

    with caplog.at_level(logging.WARNING, logger="factorlib.metrics"):
        _newey_west_t_test(np.arange(10, dtype=np.float64), lags=5)
    msgs = [r.message for r in caplog.records if r.name == "factorlib.metrics"]
